...
export function Bubble({ bubble, onClick }: BubbleProps) {
  ...
  return (
    <div
      className="bubble-float bubble-shadow absolute flex flex-col items-center"
      style={{
        left: bubble.position.x - touchSize / 2,
        top: bubble.position.y - touchSize / 2,
      } as React.CSSProperties}
    >
      <span className="text-[10px] font-medium text-warm-black/60 whitespace-nowrap mb-0.5">
        {bubble.character.name}
      </span>
      <button
        type="button"
        className="flex items-center justify-center rounded-full cursor-pointer transition-transform hover:scale-110 focus:outline-none focus-visible:ring-2 focus-visible:ring-philosopher/50 active:scale-90"
        style={{
          width: touchSize,
          height: touchSize,
